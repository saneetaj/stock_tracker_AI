...
try:
    ...
    # We'll allow historical data to be fetched even if live stream fails.
...
def compute_rsi(series: pd.Series, period: int = 14) -> pd.Series:
    """
    Compute the RSI for intraday usage with a shorter period if desired.
    """
    delta = series.diff()
...
def calculate_intraday_indicators(data: pd.DataFrame) -> pd.DataFrame:
    """
    Calculates intraday-focused indicators with shorter periods:
      - SMA_20 (20 bars)
      - RSI(7)
      - Bollinger Bands(20)
    """
    ...
    # Example: shorter SMA for intraday
    data["SMA_20"] = data["Close"].rolling(window=20).mean()

    # RSI(7) for quicker signals
...
# -------------------------
# Intraday "Buy Low, Sell High" Signal Generation

def generate_intraday_signals(data: pd.DataFrame) -> pd.DataFrame:
    """
    Generates simpler buy/sell signals for day trading:
      - Buy if RSI < 30 OR Close < Lower Bollinger
      - Sell if RSI > 70 OR Close > Upper Bollinger
    """
    data = data.copy()

    # Conditions for oversold => Buy
    data["Buy_Signal"] = (data["RSI"] < 30) | (data["Close"] < data["BB_Lower"])

    # Conditions for overbought => Sell
...
# -------------------------
# Simple Intraday Backtesting Function

def backtest_intraday_strategy(data: pd.DataFrame) -> pd.DataFrame:
    """
    Backtests a day trading approach:
      - Enter long on next bar after Buy_Signal
      - Exit on next bar after Sell_Signal
    """
    df = data.copy()
    ...
    # Create Signal column: 1 = buy, 0 = sell
    df["Signal"] = 0
    df.loc[df["Buy_Signal"], "Signal"] = 1
    df.loc[df["Sell_Signal"], "Signal"] = 0

    # Forward-fill the position, shifted by 1 to simulate entering the next bar
...
def get_intraday_stock_data(ticker: str, days: int = 1) -> Optional[pd.DataFrame]:
    """
    Fetch intraday 1-minute bars for the past 'days' from Alpaca.
    """
    try:
...
# -------------------------
# News and Sentiment Functions (unchanged, optional)
...
async def main():
    ...
    if st.button("🔍 Analyze"):
        ...
        for ticker in tickers:
            ...
            fig.update_layout(
                title=f"{ticker} Intraday Chart (1-minute) with Buy/Sell Signals",
                xaxis_title="Time",
                yaxis_title="Price",
